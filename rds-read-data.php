...
<?php
  ...
  // Connect to the RDS database за допомогою mysqli
  $link = mysqli_connect($RDS_URL, $RDS_user, $RDS_pwd, $RDS_DB) or die(mysqli_connect_error());
...
if ( $mode=="add")
 ...
 if ( $mode=="added")
 {
 mysqli_query($link, "INSERT INTO address (lastname, firstname, phone, email) VALUES ('$lastname', '$firstname', '$phone', '$email')");
 }
...
if ( $mode=="edit")
 ...
 if ( $mode=="edited")
 {
 mysqli_query($link, "UPDATE address SET lastname = '$lastname', firstname = '$firstname', phone = '$phone', email = '$email' WHERE id = $id");
...
if ( $mode=="remove")
 ...
 // Використовуємо mysqli_query та mysqli_error з лінком
 $data = mysqli_query($link, "SELECT * FROM address ORDER BY lastname ASC")
